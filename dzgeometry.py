...
def kemove_close_points(gdf, threshold, column=False, how='average'):
    '''
    ...
    for ix, x, y, flag in zip(gdf_copy.index,
        ...
        count = count + 1
        # if the points have been checked, no need to do anything
        if gdf_copy.flag[ix] == True:
            continue
        valid_copy = gdf_copy[gdf_copy.flag == False]
        points_collection = valid_copy[valid_copy.distance(
            Point(x, y)) < threshold]
...
def plot_gdf(gdf, fig=False, filename='Test', column_plot=False, label=False,
    ...
    if column_plot != False:
        ...
        sm = plt.cm.ScalarMappable(
            cmap='gnuplot2_r', norm=plt.Normalize(vmin=vmin, vmax=vmax))
        sm._A = []
        cbar = fig.colorbar(sm, cax=cax)
        # we use 3 month as the tick
        time_delta = np.timedelta64(1, 'D')*30.5/np.timedelta64(1, 's')*3
        while ticks[-1] < cbar.vmax:
            ticks.append(cbar.vmin+time_delta*i)
            i = i+1
        cbar.set_ticks(ticks)
        cbar.set_ticklabels([datetime.datetime.fromtimestamp(
            t).strftime('%d-%b-%Y') for t in ticks])
        print(cbar.vmin)
    ...
    if label != False:
        ...
        for index, row in gdf.iterrows():
            ax.annotate(s=str(row[label]), xy=(row['coords'].x,
                                               row['coords'].y), horizontalalignment='center',
                        size=labelsize)
    return fig, ax
