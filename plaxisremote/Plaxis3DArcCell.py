...
def main():
    '''
    This subroutine process the stress output from Plaxis 3D
    The vertical stresses on the planes at different levels are saved in excel files
    in a format of 'Phase 2 @ -20.00mPD' and the coordinates for the large cell
    and small cells are saved in seperate files at the folder './Input/Large Cell.xlsx'
    and './Input/Small cell.xlsx' respectively.
    we have 4 models with different seabed
    '''
...
def generate_pdf(filepath,level,phasename,stone_column,large_cell,small_cell):
    '''
    This function produce the pdfs with the data passed in.
    name of the file will be foremed using the level and phasename
    Parameters:
        filepath - path to the file
        level - the cut level of the section
        phasename - name of the model phase, in this case, the legitimate name
        of phase should be 'Phase 2', 'Phase 8', etc.
        large_cell, small_cell - a geopandas data frame holding the information
        of the large and small cell.
    Return:
        None
    '''
